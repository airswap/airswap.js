...
var delegateFactoryABI = require('./abis/delegateFactory');

var wrapperABI = require('./abis/wrapper');
...
var NODESMITH_GETH_NODE = function (N) {
  switch (N) {
    case RINKEBY_ID:
...
var alchemyWeb3 = JEST_IS_TESTING ? null : createAlchemyWeb3(ALCHEMY_WEBSOCKET_URL);
var httpProvider = new RetryProvider(AIRSWAP_GETH_NODE_ADDRESS, NETWORK);
var infuraProvider = new RetryProvider(INFURA_GETH_NODE, NETWORK);
var nodesmithProvider = new RetryProvider(NODESMITH_GETH_NODE, NETWORK); // alchemy provider has built in retry
// https://github.com/alchemyplatform/alchemy-web3

var alchemyWebsocketProvider = JEST_IS_TESTING ? null : new ethers.providers.Web3Provider(alchemyWeb3.currentProvider);
...
var NODESMITH_KEY = process.env.REACT_APP_NODESMITH_KEY || process.env.NODESMITH_KEY;
var NODESMITH_URL = "wss://ethereum.api.nodesmith.io/v1/".concat(NETWORK_NAME || 'mainnet', "/jsonrpc/ws?apiKey=").concat(NODESMITH_KEY);
...
module.exports = {
  ...
  NODESMITH_KEY: NODESMITH_KEY,
  NODESMITH_URL: NODESMITH_URL,
  ...
  NODESMITH_GETH_NODE: NODESMITH_GETH_NODE,
