...
var _require = require('./abiGen/utils'),
    ...
    getInterfaceTransactionFunctions = _require.getInterfaceTransactionFunctions; // eslint-disable-next-line
...
function generateContractFunctionMiddleware(abiLocation, contractKey) {
  ...
  var onlyCalls = _.filter(contractFunctions, {
    type: 'call'
...
function createSubmodules(_ref7) {
  ...
  fs.mkdir("./".concat(namespace.toLowerCase(), "/redux/"), {
    ...
    try {
      fs.writeFileSync("./".concat(namespace, "/redux/index.js"), generateReduxIndex(), {
        flag: 'wx'
      });
