...
function () {
  ...
  regeneratorRuntime.mark(function _callee(store, actionType, argParams) {
    var state, _argParams, args, abis, params, to, data, gasLimit, tokens, order, makerToken, takerToken, _gasSelectors$getCurr, gwei, gasPrice;
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          ...
          case 6:
            to = _context.sent;
            data = args.data;
            params = (0, _transformations.getParsedInputFromTransaction)({
              to: to,
              data: data,
              value: '0'
            }, abis);
            ...
            if (params.name === 'fill' || params.name === 'swap') {
              ...
              order = _redux.selectors.makeGetReadableOrder(state)(params.parameters);
              makerToken = order.makerToken, takerToken = order.takerToken;
              gasLimit = Math.max.apply(Math, _toConsumableArray([_lodash.default.get(_lodash.default.find(tokens, {
                address: makerToken
              }), 'gasLimit', 400000), _lodash.default.get(_lodash.default.find(tokens, {
                address: takerToken
              }), 'gasLimit', 400000)].map(Number)));
...
function connectWalletLink(store) {
  ...
  var provider = walletLink.makeWeb3Provider(_constants.AIRSWAP_GETH_NODE_ADDRESS, _constants.NETWORK);
  provider.enable().then(function () {
    signer = (0, _getSigner.default)({
      web3Provider: provider
    }, walletActions);
