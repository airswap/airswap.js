...
IPFS.prototype.sendAsync = function sendAsync(opts, cb) {
  ...
  return (cb ? noopPromise : newPromise)((resolve, reject) => {
      ...
      if (request.readyState === 4 && request.timeout !== 1) {
        if (request.status !== 200) {
          ...
          try {
            ...
            callback(
              new Error(
                `[ipfs-mini] while parsing data: '${String(request.responseText)}', error: ${String(
                  jsonError,
                )} with provider: '${self.requestBase}'`,
                null,
              ),
            )
          }
